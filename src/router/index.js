...
router.beforeEach((to, from, next) => {
  // Title will be set via globalProperties in main.js
  if (typeof window !== 'undefined' && to.meta.title) {
    const htmlTitle = store.state.configuration.htmlTitle || 'Blog'
    document.title = to.meta.title ? `${htmlTitle} - ${to.meta.title}` : htmlTitle
  }
  next()
})
